function ww = marius_dw(Ms, param)
...
weights = 0.01*ones(1,n1*(n1-1)/2);
lambda = 5;
lambdas = lambda*ones(size(weights));
...
%         [M,spc_affinities] = getMatchingMatrix(vec{pair}, spc_i_indices{pair}, spc_j_indices{pair}, labels{pair}, nodes{pair}, 0.5, w);
        maskw = zeros(n1,n1);
        maskw(~tril(ones(n1))) = w;
        maskw = maskw + maskw';
        M = Mvec{pair} .* repmat(maskw,n2,n2);
...
%             dM = Mvec{pair,var};
            ...
            mask(var2ind(var,:)) = 1;
            mask = mask + mask';
            dM = Mvec{pair} .* repmat(mask,n2,n2);
...
%     w = w / (norm(w)+eps);
    warch{iter} = w;
    bv(iter) = aux_vec' * v + lambdas(:)'*w(:);
end

E12 = ones(n1,n2);
[L12(:,1) L12(:,2)] = find(E12);
[group1 group2] = make_group12(L12);
yraw = RRWM(double(sum(Ms{1},3)), group1, group2);
ysol = greedyMapping(yraw, group1, group2);

gt'*ysol/sum(gt)
gt'*logical(aux_vec)/sum(gt)
